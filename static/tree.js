// SFS College Dream Tree — Cinematic Banyan Animation
...
class BorderDreamTree {
    ...
    /** Called by triggerActivation() after the 3-second logo countdown */
    startSeedSequence() {
    ...
    // ──────────────────────────────────────────────────────────────────────────
    //  Bezier helpers (kept from original)
    // ──────────────────────────────────────────────────────────────────────────
    ...
    // ──────────────────────────────────────────────────────────────────────────
    //  Message system (kept from original, adjusted for banyan)
    // ──────────────────────────────────────────────────────────────────────────
    get9Positions() {
        const W = this.width, H = this.height;
        const positions = [];
    ...
    drawSketchLeaf(ctx, x, y, size, color, angle) {
        ...
        // Faint semi-transparent dark center mask (hides background lines slightly)
        ctx.beginPath();
        ctx.moveTo(0, -size);
    ...
    drawCordateLeaf(ctx, bx, by, size, angle, color, pLeaf) {
        ...
        // 1. Faint masking fill behind leaf outline to hide stars
        ctx.beginPath();
        ctx.moveTo(0, 0);
        // Left side curve (draws a heart-shaped outline lobe)
        ctx.bezierCurveTo(-s * 0.4, s * 0.15, -s * 0.75, -s * 0.2, -s * 0.65, -s * 0.55);
        ctx.bezierCurveTo(-s * 0.55, -s * 0.8, -s * 0.2, -s * 0.95, 0, -s);
        // Right side curve (symmetric)
        ...
        // 4. Lateral Veins (3-4 on each side, angled outwards and curving slightly up)
        ctx.lineWidth = 0.7;
        const numVeins = 3;
        for (let i = 1; i <= numVeins; i++) {
            ...
            const vAngle = 0.5; // radians (about 28 degrees)
    ...
    drawLanceolateLeaf(ctx, bx, by, size, angle, color, pLeaf) {
        ...
        // 4. Lateral Veins (4 on each side pointing upwards)
        ctx.lineWidth = 0.7;
        const numVeins = 4;
        for (let i = 1; i <= numVeins; i++) {
            const vFactor = i / (numVeins + 0.8);
    ...
    drawSeedling(ctx) {
        ...
        // ────────────────────────────── 1. Pebble Mound (drawn at the bottom)
        ...
        // ────────────────────────────── 4. Trunk / Stem (Double-line tapered outline)
        ...
        if (stemP > 0) {
            // Width tapers from 7px at bottom to 3.5px at top
            this.drawOutlineCurve(ctx, stemPoints, (t) => 7 - t * 3.5, color, 0.85);
        }

        // ────────────────────────────── 5. Leaf Growth (Exactly 5 lanceolate leaves matching sketch reference)
        const leafSpecs = [
            // 1. Lower Left Leaf (start p > 0.22, full at 0.58)
            {
                name: 'lower-left',
            ...
            // 2. Middle Right Leaf (start p > 0.35, full at 0.7)
            {
                name: 'middle-right',
            ...
            // 3. Upper Left Leaf (start p > 0.46, full at 0.8)
            {
                name: 'upper-left',
            ...
            // 4. Upper Right Leaf (start p > 0.56, full at 0.88)
            {
                name: 'upper-right',
            ...
            // 5. Top Center Leaf (start p > 0.68, full at 1.0)
            {
                name: 'top',
        ...
        leafSpecs.forEach(spec => {
            if (p > spec.startP) {
                ...
                // Draw the lanceolate leaf at the end of the petiole
    ...
    // ──────────────────────────────────────────────────────────────────────────
    //  STAGE: Full Banyan Tree  (growing + title + raining)
    // ──────────────────────────────────────────────────────────────────────────
    drawBanyanTree(ctx, messageOpacity = 1.0) {
        ...
        const h = this.treeHue;
        const p = this.treeProgress; // 0→1 during growing
        ...
        // ────────────────────────────── 2. Trunk (Morphing from Seedling Stem to Hourglass Banyan Trunk)
        const tMorph = Math.min(1, p / 0.45);
        const trunkH = groundY - trunkTopY;
        const curH = 175 + (trunkH - 175) * p; // grows from seedling stem height to banyan trunk height
        const curBaseY = groundY - 14 * (1 - tMorph); // morphs from seedling stem base (groundY - 14) to banyan groundY
        ...
        // Faint masking fill behind trunk outline to cover background stars
        ctx.beginPath();
        ...
        // Fluted inner vertical strands (textured bark strands following hourglass curves)
        const numStrands = 6;
        ...
        // ────────────────────────────── 3. Main Branches (Twisting outline branches)
        if (p > 0.3) {
            ...
            branchEnds.forEach((end, idx) => {
                const startX = cx + (idx - 1.5) * curTopW * 0.45; // attachment point on morphing trunk top
                ctx.beginPath();
        ...
        // ────────────────────────────── 5. Aerial Roots (hanging vertically down)
        if (p > 0.6) {
        ...
        // ────────────────────────────── 6. Border Accent Leaves (drawn as outlines)
        ...
        if (this.activationStage === 'raining' || this.activationStage === 'title' || this.activationStage === 'ended') {
            // First pass: Draw all static canopy leaves (unplucked or waiting on delay)
            this.canopyLeaves.forEach(l => {
                if (!l.isPlucked) {
                    l.offset += l.pulseSpeed;
            ...
            // Second pass: Draw all active message leaves (drawn ON TOP of everything)
            this.canopyLeaves.forEach(l => {
                if (l.isPlucked && l.wish && l.delay <= 0) {
                    l.lifetime++;

                    // 240 frames total: 30 frames fade-in, 180 frames visible, 30 frames fade-out
                    let opacity = 1;
                    if (l.lifetime < 30) {
                    ...
                    // Recycle when lifetime is complete
                    ...
                    // Update palette hue to match the current tree hue
...
function triggerActivation() {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ type: 'keypress_activation' }));
    }
    ...
    if (splash && logoImg) {
        ...
        const targetY = window.innerHeight * 0.88 - 14; // matches tree.js bottom seed position
    ...
    // Start seed sequence slightly before logo completely fades to 0
    setTimeout(() => {
        tree.startSeedSequence();
    }, 2600);

    // Hide splash container fully at 3 seconds
    setTimeout(() => {
...
function handleKeyPress(event) {
    ...
    if (tree && pressedKey === tree.activationKey && !tree.isActivated) {
